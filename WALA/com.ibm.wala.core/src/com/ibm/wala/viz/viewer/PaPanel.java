 ...
 * viewer on the right. Sets the IR of and pc according to the chosen pointer/instance key when
 * possible (e.g., allocation side for NormalAllocationInNode instance keys. Can be customized for
 * clients that use different their own pointer / instance keys (see JsPaPanel)
 *
 * <p>Note the two steps initialization require (calling init())
 *
 * @author yinnonh
 */
public class PaPanel extends JSplitPane {
  ...
  /**
   * Two steps initialization is required here is our deriver can choose the roots for the heap
   * tree.
   */
  public void init() {
    ...
    heapTree.addTreeExpansionListener(
   ...
   * Override if you have special pointer keys (not just for fields)
   */
