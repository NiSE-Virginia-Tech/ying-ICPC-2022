 ...
 * {@link TabulationSolver tabulation}.
 */
public class ContextSensitiveReachingDefs {
  ...
  private class ReachingDefsFlowFunctions
    ...
    /**
     * flow function from call node to return node at a call site when callees exist. We kill
     * everything; surviving facts should flow out of the callee
     */
    @Override
  ...
  /**
   * Definition of the reaching definitions tabulation problem. Note that we choose to make the
   * problem a <em>partially</em> balanced tabulation problem, where the solver is seeded with the
   * putstatic instructions themselves. The problem is partially balanced since a definition in a
   * callee used as a seed for the analysis may then reach a caller, yielding a "return" without a
   * corresponding "call." An alternative to this approach, used in the Reps-Horwitz-Sagiv POPL95
   * paper, would be to "lift" the domain of putstatic instructions with a 0 (bottom) element, have
   * a 0->0 transition in all transfer functions, and then seed the analysis with the path edge
   * (main_entry, 0) -&gt; (main_entry, 0). We choose the partially-balanced approach to avoid
   * pollution of the flow functions.
   */
  private class ReachingDefsProblem
    ...
    @Override
    public TabulationDomain<Pair<CGNode, Integer>, BasicBlockInContext<IExplodedBasicBlock>>
        getDomain() {
      return domain;
    }
  ...
  public TabulationResult<BasicBlockInContext<IExplodedBasicBlock>, CGNode, Pair<CGNode, Integer>>
      analyze() {
    PartiallyBalancedTabulationSolver<
            BasicBlockInContext<IExplodedBasicBlock>, CGNode, Pair<CGNode, Integer>>
        solver =
            PartiallyBalancedTabulationSolver.createPartiallyBalancedTabulationSolver(
                new ReachingDefsProblem(), null);
