 ...
 *
 * <p>The basic algorithm here is an abstract interpretation over the Java bytecode to determine
 * types of stack locations and local variables. As a side effect, the flow functions of the
 * abstract interpretation emit instructions, eliminating the stack abstraction and moving to a
 * register-transfer language in SSA form.
 */
public class DexSSABuilder extends AbstractIntRegisterMachine {
  public static DexSSABuilder make(
      DexIMethod method,
      SSACFG cfg,
      DexCFG scfg,
    ...
    return new DexSSABuilder(
        method, cfg, scfg, instructions, symbolTable, buildLocalMap, piNodePolicy);
  }
  ...
  private DexSSABuilder(
    ...
    localMap =
        buildLocalMap
            ? new SSA2LocalMap(
                scfg, instructions.length, cfg.getNumberOfNodes(), method.getMaxLocals())
            : null;
    init(
        new SymbolTableMeeter(cfg, scfg),
        new SymbolicPropagator(scfg, instructions, cfg, piNodePolicy));
    this.method = method;
  ...
  protected void initializeVariables() {
    ...
    // can't use just getNumberOfParameters because it does not account for Long/Wide variables
    // which take up 2 registers
    // as the parameter
  ...
  /**
   * This class defines the type abstractions for this analysis and the flow function for each
   * instruction in the ShrikeBT IR.
   */
  private class SymbolicPropagator extends BasicRegisterFlowProvider {
    ...
    /**
     * If we've already created the current instruction, return the value number def'ed by the
     * current instruction. Else, create a new symbol.
     */
    ...
    /**
     * If we've already created the current instruction, return the value number representing the
     * exception the instruction may throw. Else, create a new symbol
     */
    ...
    class NodeVisitor extends BasicRegisterMachineVisitor {
      ...
      public void visitArrayGet(ArrayGet instruction) {
        ...
        //              } else {
        emitInstruction(
            insts.ArrayLoadInstruction(getCurrentInstructionIndex(), result, arrayRef, index, t));
        //              }
      }
      ...
      public void visitArrayPut(ArrayPut instruction) {
        ...
        //              int index = workingState.pop();
        //              int arrayRef = workingState.pop();
        //              TypeReference t = ShrikeUtil.makeTypeReference(loader,
        // instruction.getType());
        emitInstruction(
      ...
      public void visitArrayFill(ArrayFill instruction) {
        ...
        while (iae.hasNext()) {
          ...
          int value;
          //
          //                      System.out.println("Index: " + ae.bufferIndex + ", Width: " +
          // ae.elementWidth + ", Value: " +  byte_buffer.getSomethingDependingonType );

          // okay to call the getConstant(String) for a char?
          if (t.equals(TypeReference.Char)) value = symbolTable.getConstant((char) ae.intValue());
          else if (t.equals(TypeReference.Byte)) value = symbolTable.getConstant(ae.byteValue());
          else if (t.equals(TypeReference.Short)) value = symbolTable.getConstant(ae.shortValue());
          else if (t.equals(TypeReference.Int)) value = symbolTable.getConstant(ae.intValue());
          else if (t.equals(TypeReference.Long)) value = symbolTable.getConstant(ae.longValue());
          else if (t.equals(TypeReference.Float)) value = symbolTable.getConstant(ae.floatValue());
          ...
          //                  System.out.println("Index: " + t.bufferIndex + ", Value: " +
      ...
      public void visitBinaryOperation(BinaryOperation instruction) {
        ...
        setLocal(dest, result);
        //              workingState.push(result);
        //              boolean isFloat = instruction.getType().equals(TYPE_double) ||
        // instruction.getType().equals(TYPE_float);
        emitInstruction(
            insts.BinaryOpInstruction(
                getCurrentInstructionIndex(),
                instruction.getOperator(),
                false,
                instruction.isUnsigned(),
                result,
                val1,
                val2,
                !instruction.isFloat()));
      ...
      public void visitConstant(Constant instruction) {
        ...
        if (instruction instanceof Constant.ClassConstant) {
          ...
          //                  symbol =
          // symbolTable.getConstant(((Constant.ClassConstant)instruction).value);
        ...
        //              Language l =
        // cfg.getMethod().getDeclaringClass().getClassLoader().getLanguage();
      ...
      //              workingState.push(result);
      //
      //              TypeReference fromType = ShrikeUtil.makeTypeReference(loader,
      // instruction.getFromType());
      //              TypeReference toType = ShrikeUtil.makeTypeReference(loader,
      // instruction.getToType());
      //
      //              emitInstruction(insts.ConversionInstruction(result, val, fromType, toType,
      ...
      public void visitInvoke(Invoke instruction) {
        ...
        MethodReference m =
            MethodReference.findOrCreate(
                lang,
                loader,
                instruction.clazzName,
                instruction.methodName,
                instruction.descriptor);

        // ((DexIClass)dexCFG.getDexMethod().getDeclaringClass()).getSuperclass().get
        //              MethodReference m =
        // ((DexIClass)dexCFG.getDexMethod().getDeclaringClass()).loader.lookupClass(TypeName.findOrCreate(instruction.clazzName)).
        ...
        for (int i = 0; i < m.getNumberOfParameters(); i++)
          if (m.getParameterType(i) == TypeReference.Double
              || m.getParameterType(i) == TypeReference.Long) n--;
        int[] params = new int[n];
        ...
        if (n == m.getNumberOfParameters()) {
          for (int i = 0; i < n; i++) {
            ...
            if (m.getParameterType(i) == TypeReference.Double
                || m.getParameterType(i) == TypeReference.Long) arg_i++;
            arg_i++;
          }
        }
        ...
        else throw new UnsupportedOperationException("visitInvoke DexSSABuilder, error");

        if (m.getReturnType().equals(TypeReference.Void)) {
          SSAInstruction inst =
              insts.InvokeInstruction(getCurrentInstructionIndex(), params, exc, site, null);
          ...
          SSAInstruction inst =
              insts.InvokeInstruction(
                  getCurrentInstructionIndex(), result, params, exc, site, null);
      ...
      //                  }
      //
      //                  TypeReference type = ShrikeUtil.makeTypeReference(loader,
      // instruction.getType());
      ...
      //          /*
      //           * @see
      // com.ibm.wala.shrikeBT.IInstruction.Visitor#visitLocalStore(com.ibm.wala.shrikeBT.StoreInstruction)
      //           */
      //          @Override
      //          public void visitLocalStore(IStoreInstruction instruction) {
      //              if (localMap != null) {
      //                  localMap.startRange(getCurrentInstructionIndex(),
      // instruction.getVarIndex(), workingState.peek());
      //              }
      ...
      public void visitNew(New instruction) {
        ...
        //                      sizes[instruction.getArrayBoundsCount() - 1 - i] =
        // workingState.pop();
        //                  }
        //                  emitInstruction(insts.NewInstruction(result, ref, sizes));
        //              } else {
        emitInstruction(
            insts.NewInstruction(getCurrentInstructionIndex(), result, instruction.newSiteRef));
        //                  popN(instruction);
        //              }
        setLocal(dest, result);
        //              workingState.push(result);
      ...
      public void visitNewArray(NewArray instruction) {
        ...
        emitInstruction(
            insts.NewInstruction(
                getCurrentInstructionIndex(), result, instruction.newSiteRef, sizes));
        setLocal(dest, result);
      }
      ...
      //              int result = reuseOrCreateDef();
      //              workingState.push(result);
      //              emitInstruction(insts.BinaryOpInstruction(instruction.getOperator(), false,
      // instruction.isUnsigned(), result, val1, val2,
      //                      true));
      //          }
      ...
      public void visitSwitch(Switch instruction) {
        ...
        //              int val = workingState.pop();
        // TODO: figure out if the switch offset should refer to a pc offset or an instruction id or
        // what
        emitInstruction(
            insts.SwitchInstruction(
                getCurrentInstructionIndex(),
                val,
                instruction.getDefaultLabel(),
          ...
          if (s == null) {
            ...
            s =
                insts.GetCaughtExceptionInstruction(
                    newBB.getLastInstructionIndex(), bbidx, exceptionValue);
        ...
        if (instruction.isConversion()) {
          ...
          emitInstruction(
              insts.ConversionInstruction(
                  getCurrentInstructionIndex(), result, val, fromType, toType, overflows));
        } else {
          if (instruction.op == UnaryOperation.OpID.MOVE) {
            ...
            emitInstruction(
                insts.UnaryOpInstruction(
                    getCurrentInstructionIndex(), instruction.getOperator(), result, val));
          }
        }
      }
      ...
      //                  ssaIndirections.setUse(getCurrentInstructionIndex(), new
      // ShrikeLocalName(locals[i]), workingState.getLocal(locals[i]));
      //              }
      //          }
      ...
      // doIndirectReads(bytecodeIndirections.indirectlyReadLocals(getCurrentInstructionIndex()));
      //              TypeReference t = ShrikeUtil.makeTypeReference(loader,
      // instruction.getPushedType(null));
      ...
      //                      ssaIndirections.setDef(idx, name, rval==-1? symbolTable.newSymbol():
      // rval);
      //                  }
    ...
    // if ((addPiForFieldSelect) && (creators.length > val) && (creators[val] instanceof
    // SSAGetInstruction)
    ...
    // && (((SSAInvokeInstruction) creators[val]).getInvocationCode() ==
    ...
    private void maybeInsertPi(SSAConditionalBranchInstruction cond) {
      if (piNodePolicy != null) {
        Pair<Integer, SSAInstruction> pi =
            piNodePolicy.getPi(cond, getDef(cond.getUse(0)), getDef(cond.getUse(1)), symbolTable);
        if (pi != null) {
          reuseOrCreatePi(pi.snd, pi.fst);
        }
      }
  ...
  private static class SSA2LocalMap implements com.ibm.wala.ssa.IR.SSA2LocalMap {
    ...
    /**
     * Record the beginning of a new range, starting at the given program counter, in which a
     * particular value number corresponds to a particular local number
     */
    @SuppressWarnings("unused")
     ...
     * @return if we know that immediately after the given program counter, v_vn corresponds to some
     *     set of locals, then return an array of the local numbers. else return null.
     */
