...
public abstract class AbstractIntRegisterMachine implements FixedPointConstants {
  ...
  public class MachineState extends AbstractVariable<MachineState> {
    ...
    public void replaceValue(int from, int to) {
      ...
      if (locals != null) for (int i = 0; i < maxLocals; i++) if (locals[i] == from) locals[i] = to;
    }

    public boolean hasValue(int val) {
      if (stack != null) for (int i = 0; i < stackHeight; i++) if (stack[i] == val) return true;

      if (locals != null) for (int i = 0; i < maxLocals; i++) if (locals[i] == val) return true;
  ...
  protected abstract static class BasicRegisterFlowProvider implements FlowProvider, DexConstants {
    ...
    protected static class BasicRegisterMachineVisitor extends Visitor {
      ...
      public void visitInvoke(Invoke instruction) {
        ...
        //              TypeReference returnType = ShrikeUtil.makeTypeReference(loader,
        // Util.getReturnType(instruction.getMethodSignature()));
